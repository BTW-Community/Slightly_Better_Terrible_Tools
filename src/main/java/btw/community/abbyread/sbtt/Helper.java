...
public class Helper {
    ...
    public static boolean loosen(ItemStack stack, Block block, int meta, World world, int x, int y, int z, int fromSide) {
        if (BlockTags.is(block, meta, BlockTag.LOOSE)) return false;

        boolean swapped = false;
        Block newBlock = null;
        int newMeta = meta;

        if (BlockTags.isButNot(block, 0, BlockTag.DIRTLIKE)) {
            if (block == Block.dirt) newBlock = BTWBlocks.looseDirt;
            if (block == BTWBlocks.dirtSlab) newBlock = BTWBlocks.looseDirtSlab;
            if (BlockTags.isAll(block, meta, BlockTag.SPARSE, BlockTag.CUBE)) newBlock = BTWBlocks.looseSparseGrass;
            if (block == BTWBlocks.grassSlab) newBlock = BTWBlocks.looseSparseGrassSlab;
        }
